...
<body>
    ...
    <form action="responsi.php" method="post">
       <h2><input type="submit" value="Maps Persebaran Halte"></h2> 
    </form>
    ...
    <script>
        ...
        title.update = function () {
            this._div.innerHTML = '<h2>LATIHAN WEBGIS | LULUSAN SD SEDERAJAT KABUPATEN SLEMAN</h2>MATAKULIAH PEMROGRAMAN SPASIAL : WEB'
        };
        title.addTo(map);
    ...
    <script>
        ...
        $.getJSON("geoserver.php", function (data) {
            ...
            wfsgeoserver1.eachLayer(function (layer) {
                var TAMAT_SD_S = layer.feature.properties.TAMAT_SD_S;

                // Apply styling based on jumlah property
                if (TAMAT_SD_S >= 0 && TAMAT_SD_S <= 500) {
                    layer.setStyle({
                        fillColor: '#ffffff',
                        color: '#232323',
                        weight: 1
                    });
                } else if (TAMAT_SD_S > 501 && TAMAT_SD_S <= 1000) {
                    layer.setStyle({
                        fillColor: '#ff8080',
                        color: '#232323',
                        weight: 1
                    });
                } else if (TAMAT_SD_S > 1001 ) {
                    layer.setStyle({
                        fillColor: '#ff0000',
                        color: '#232323',
                        weight: 1
    ...
    <script>
        L.Control.Legend = L.Control.extend({
            onAdd: function (map) {
                ...
                img.src = 'assets/img/legend/simbol.png';
        ...
        <?php
            ...
            if ($result -> num_rows > 0) {
                while ($row = $result -> fetch_assoc()) {
                    ...
                    $long = $row["longitude"];
                    $halte = $row["halte"];
                    echo "L.marker([$lat, $long]).addTo(map).bindPopup('GNama $halte')";
                }
            }
            else {
                echo "0 results";
